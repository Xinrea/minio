...
// this map is used to cover all S3Errors
// enum class S3Errors
// {
//   //From Core//
//   //////////////////////////////////////////////////////////////////////////////////////////
...
//	};
var injectMap = map[string]APIErrorCode{
	"INCOMPLETE_SIGNATURE":          ErrIncompleteSignature,
	"INTERNAL_FAILURE":              ErrInternalError,
	"INVALID_ACTION":                ErrInvalidAction,
	"INVALID_CLIENT_TOKEN_ID":       ErrInvalidClientTokenID,
	"INVALID_PARAMETER_COMBINATION": ErrInvalidParameterCombination,
	"INVALID_QUERY_PARAMETER":       ErrInvalidQueryParameter,
	"INVALID_PARAMETER_VALUE":       ErrInvalidParameterValue,
	"OPT_IN_REQUIRED":               ErrOptInRequired,
	"REQUEST_EXPIRED":               ErrRequestExpired,
	"SERVICE_UNAVAILABLE":           ErrServiceUnavailable,
	"THROTTLING":                    ErrThrottling,
	"VALIDATION":                    ErrValidation,
	"ACCESS_DENIED":                 ErrAccessDenied,
	"RESOURCE_NOT_FOUND":            ErrResourceNotFound,
	"UNRECOGNIZED_CLIENT":           ErrUnrecognizedClient,
	"MALFORMED_QUERY_STRING":        ErrMalformedQueryString,
	"SLOW_DOWN":                     ErrSlowDown,
	"REQUEST_TIME_TOO_SKEWED":       ErrRequestTimeTooSkewed,
	"INVALID_SIGNATURE":             ErrInvalidSignature,
	"SIGNATURE_DOES_NOT_MATCH":      ErrSignatureDoesNotMatch,
	"INVALID_ACCESS_KEY_ID":         ErrInvalidAccessKeyID,
	"REQUEST_TIMEOUT":               ErrRequestTimeout,
	"NETWORK_CONNECTION":            ErrNetworkDisconnected,
	"UNKNOWN":                       ErrUnknownError,
	"BUCKET_ALREADY_EXISTS":         ErrBucketAlreadyExists,
	"BUCKET_ALREADY_OWNED_BY_YOU":   ErrBucketAlreadyOwnedByYou,
	"INVALID_OBJECT_STATE":          ErrInvalidObjectState,
	"NO_SUCH_BUCKET":                ErrNoSuchBucket,
	"NO_SUCH_KEY":                   ErrNoSuchKey,
...
func (api objectAPIHandlers) errorInjection(ctx context.Context, objectAPI ObjectLayer, bucket, object string, w http.ResponseWriter, r *http.Request) bool {
	eBucket := os.Getenv("EBUCKET")
	eFile := os.Getenv("EFILE")
	eError := os.Getenv("EERR")

	// handle typo in env variable name
	if eError == "" {
		eError = os.Getenv("ERRR")
	}

	// not fully set, return
	if eFile == "" || eError == "" {
		return false
	}

	// not for this bucket, return
	if eBucket != "" && eBucket != bucket {
		return false
	}

	// not for this object, return
	if eFile != object {
		return false
	}

	if injectErr, ok := injectMap[eError]; ok {
		fmt.Printf("[ErrorInjection] Injecting error: %s file=%s\n", eError, eFile)
		writeErrorResponse(ctx, w, errorCodes.ToAPIErr(injectErr), r.URL)
		return true
	} else {
		fmt.Printf("[ErrorInjection] Unknown error code: %s\n", eError)
	}
...
func (api objectAPIHandlers) getObjectHandler(ctx context.Context, objectAPI ObjectLayer, bucket, object string, w http.ResponseWriter, r *http.Request) {
	...
	if api.errorInjection(ctx, objectAPI, bucket, object, w, r) {
		return
	}
